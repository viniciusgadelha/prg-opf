...
if __name__ == "__main__":
    ...
    for p in np.arange(0.05, 3.55, 0.05):
        ...
        terminal_port = pd.read_csv(case + 'terminal_ports.csv')
        terminal_port.loc[0, 'p_mw'] = terminal_port.loc[
                                           0, 'p_mw'] + p  # increment of power to the terminal port

        # terminal_port.loc[0, 'q_mvar'] = terminal_port.loc[
        #                                    0, 'p_mw'] + p  # increment of power to the terminal port
        terminal_port.to_csv(case + 'terminal_ports_variation.csv', index=False)

        model = define_parameters_variables(model, data, case, DC=dc, load_variation=True)
        ...
        if count == 0:
            ...
            df_v.rename(columns={'V [kV^2]': 'V [kV^2], K=' + str(p)}, inplace=True)

            df_p = pd.read_excel(os.getcwd() + '/results/optimization_results.xlsx', sheet_name='active_powers', index_col=0)
            df_p.dropna(inplace=True)
            df_p.rename(columns={'P [MW]': 'P [MW], K=' + str(p)}, inplace=True)
            ...
            df_p.rename(columns={'Q [MVAR]': 'Q [MVAR], K=' + str(p)}, inplace=True)

            df_i = pd.read_excel(os.getcwd() + '/results/optimization_results.xlsx', sheet_name='current', index_col=0)
            df_i.dropna(inplace=True)
            df_p.rename(columns={'I [kA^2]': 'I [kA^2], K=' + str(p)}, inplace=True)
            ...
            df_p.rename(columns={'solution': 'Loss [MW], K=' + str(p)}, inplace=True)
            ...
            sens_df = pd.DataFrame(index=[p], columns=[watch_list])
            ...
            df_v_add.rename(columns={'V [kV^2]': 'V [kV^2], K=' + str(p)}, inplace=True)
            ...
            df_p_add.rename(columns={'P [MW]': 'P [MW], K=' + str(p)}, inplace=True)
            ...
            df_q_add.rename(columns={'Q [MVAR]': 'Q [MVAR], K=' + str(p)}, inplace=True)
            ...
            df_i_add.rename(columns={'I [kA^2]': 'I [kA^2], K=' + str(p)}, inplace=True)
            ...
            df_losses_add.rename(columns={'solution': 'Loss [MW], K=' + str(p)}, inplace=True)
            df_losses = pd.concat([df_losses, df_losses_add.iloc[:, 1]], axis=1)

            sens_df_add = pd.DataFrame(index=[p], columns=[watch_list])
